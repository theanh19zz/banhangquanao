...
function addToCart(productId, inputQuantity = 1, size = null, color = null) {
    let product = products.find(p => p.id == productId);
    if (product) {
        let existingProduct = cart.find(p => p.id == productId && p.size === size && p.color === color);
        if (existingProduct) {
            existingProduct.quantity += inputQuantity;
        } else {
            let productWithQuantity = { ...product, quantity: inputQuantity, size: size, color: color };
...
function addCartToHTML() {
    ...
    cart.forEach((product, index) => {
        let priceStr = product.price.replace('$', '').replace('đ', '');
        let price = parseFloat(product.price.includes('đ') ? priceStr.replace(/\./g, '') : priceStr) || 0;
            ...
            <div class="cart_product_info">  
                <div class="top_card">
                    <div class="left_card">
                        ...
                        <div class="variant_info" style="font-size: 11px; color: #666; margin-top: 4px;">
